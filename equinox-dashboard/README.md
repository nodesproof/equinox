...
client/src/components/       Layout (live shell, WalletButton), Banner (RpcBanner), MetricCard, PoolCard, SigmaChart (JSX port of web/src/ui/svg.ts — no recharts; order or block axis, Settled markers, ≈ time labels), SigmaTimeline (Activity: SigmaChart in block mode), EventsTable (full event feed with ≈ time, actor, show more), ParityPanel, EventsPreview, BoardSummary, BoardTable (one board panel: full series table, expand, Trade link), SeriesDetail (Greeks per pool + K5 prices), PoolSwitch (radiogroup per POOL_KEYS with the asset word), AmountInput (decimal input + suffix + max), PreviewLine (value / error / … / idle), TxLog (aria-live log, ✓/✗/…, explorer links), primitives (AppMark, SectionHeading, EmptyValue, SkeletonLine, StatusPill, MiniSparkline), ErrorBoundary, ui/* (shadcn: button, badge, input, tooltip, sonner, select, tabs, dialog, skeleton, table)
client/src/lib/              sync (connecting|live|stale|failed + banner model), format (grouping, bps → %, exact WAD, build stamp), boards (cell semantics of web/src/panels/board.ts, filters, footnote), route (query-in-hash links, parsers, useHashRoute/useHashQuery), trade (classic preview/log sentences, select options, LP value, faucet links), activity (event filters, block → ≈ time, Settled markers, scan notes), contracts (manifest address rows, Sourcify/commit links, cfg()/params() rows with units, docs, honesty sentences)
