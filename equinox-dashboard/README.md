...
State of this folder: the shell (sidebar, topbar, testnet strip, footer), the Overview layout and the hash router are in place; every on-chain number renders as `—` with a reason until the snapshot hooks land. Boards, Trade, Portfolio, Activity and Contracts are placeholders. Nothing on screen is fabricated.
...
client/src/App.tsx           ThemeProvider (dark) → TooltipProvider → Toaster → Router(useHashLocation) → Layout → routes
client/src/components/       Layout (shell), primitives (AppMark, SectionHeading, EmptyValue, MetricCard, StatusPill, MiniSparkline), ErrorBoundary, ui/* (shadcn: button, badge, input, tooltip, sonner, select, tabs, dialog, skeleton, table)
client/src/pages/            Overview, ComingSoon (placeholder for the routes not built yet)
client/src/index.css         Tailwind 4 + the "dark observatory" design system (custom classes)
test/                        vitest + jsdom (test/setup.ts loads jest-dom matchers)
