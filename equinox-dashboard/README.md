...
State of this folder: the shell (sidebar, topbar, testnet strip, RPC banner, footer), the Overview, the Boards page, the Trade page and the Portfolio page are **live** on `ChainProvider` (one block-pinned snapshot per 15-s poll, parity, gas, events; wallet actions through `useTrade`, one at a time, every one simulated first); loading, first-load error, stale and live states are explicit (`—` + reason, never a fabricated number), and nothing is shown for an account that is not connected. Activity and Contracts are placeholders until Task 6.
...
client/src/components/       Layout (live shell, WalletButton), Banner (RpcBanner), MetricCard, PoolCard, SigmaChart (JSX port of web/src/ui/svg.ts — no recharts), ParityPanel, EventsPreview, BoardSummary, BoardTable (one board panel: full series table, expand, Trade link), SeriesDetail (Greeks per pool + K5 prices), PoolSwitch (radiogroup per POOL_KEYS with the asset word), AmountInput (decimal input + suffix + max), PreviewLine (value / error / … / idle), TxLog (aria-live log, ✓/✗/…, explorer links), primitives (AppMark, SectionHeading, EmptyValue, SkeletonLine, StatusPill, MiniSparkline), ErrorBoundary, ui/* (shadcn: button, badge, input, tooltip, sonner, select, tabs, dialog, skeleton, table)
client/src/lib/              sync (connecting|live|stale|failed + banner model), format (grouping, bps → %, exact WAD, build stamp), boards (cell semantics of web/src/panels/board.ts, filters, footnote), route (query-in-hash links, parsers, useHashRoute/useHashQuery), trade (classic preview/log sentences, select options, LP value, faucet links)
client/src/pages/            Overview (live), Boards (live), Trade (live), Portfolio (live), ComingSoon (placeholder for the routes not built yet)
