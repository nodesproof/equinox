...
> Stylus wins on loop-heavy work — `impliedVol` 20 iterations 223,373 vs 616,839 gas (2.8×), `markPortfolio` 32 series 427,448 vs 1,242,892 (2.9×) — and loses on small single calls (`normCdf` 8,327 vs 5,435 gas cached, ≈ 0.65×; ≈ 34–35k uncached because of the ≈ 26k program-init premium). 256-bit fixed-point arithmetic itself costs ≈ 3× more in WASM (`I256` 105 gas per mul+div pair vs ≈ 30–40 in the EVM); the savings come from control flow, loops and ABI handling. Not 10×.
...
> Body (Indonesian): environment, the full devnode table, the Sepolia cached table, the end-to-end pool rows with their measurement caveats, the Sepolia rows with the shared-engine caveat and the out-of-gas incident, and the marginal-cost micro-benchmark (`I256` 105 / `i128` 29 / `u64` 0.5 gas per pair).
