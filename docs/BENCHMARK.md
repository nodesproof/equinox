...
> **Summary (English).** Gas of the same Black-Scholes math in two bit-identical implementations — `BlackScholesSol` (Solidity, PRBMath v4, via-IR) and `bs-stylus` (Rust/WASM on Arbitrum Stylus) — measured with `gasleft()` around an identical `STATICCALL` on a local Nitro devnode (ArbOS 61 / Stylus v3, `tools/bench/bench.sh`), and re-measured on Arbitrum Sepolia with the program cached: the nine Sepolia cells equal the devnode-derived "cached" column exactly.
> Stylus wins on loop-heavy work — `impliedVol` 20 iterations 223,373 vs 616,839 gas (2.8×), `markPortfolio` 32 series 427,448 vs 1,242,892 (2.9×) — and loses on small single calls (`normCdf` 8,327 vs 5,435 gas cached, ≈ 0.65×; ≈ 34–35k uncached because of the ≈ 26k program-init premium).
> 256-bit fixed-point arithmetic itself costs ≈ 3× more in WASM (`I256` 105 gas per mul+div pair vs ≈ 30–40 in the EVM; `i128` 29, `u64` 0.5); the savings come from control flow, loops and ABI handling. Not 10× — the 10× target of PRD v1.0 was retracted.
> The "cached" column was first derived on the devnode (measured − 26,296 gas, `programInitGas` 30,919 uncached vs 4,623 cached, because the devnode's CacheManager is a stub) and then **measured directly on Arbitrum Sepolia** with the program actually cached (`cargo stylus cache bid … 0`): all nine cells equal the derived column to the gas unit, and the Solidity column is identical too.
> At the transaction level the difference almost vanishes: two identical pools on the devnode (two vol engines, static mock feed) give `buy` 387,211 vs 367,932 (1.05×), `close` 254,792 vs 235,533 (1.08×), `deposit` with 6 open series 219,538 vs 220,352 (0.99×) — storage and token transfers dominate, math is ≈ 23–27 % of a `buy`.
> The Sepolia pool rows (1.11× / 1.25×) are **not** comparable with the devnode rows: both live pools share one vol engine (decision K4), so A also pays Stylus `sqrt`, B no longer pays the init premium, and only the two pricing calls differ; one `buy` pair is additionally skewed by an unequal `poke` path. Every row's return bytes are identical on both implementations; 20 further bit-exact checks against the Python spec pass on Sepolia (`tools/bench/onchain-check.sh`).
> Body (Indonesian): environment, the full devnode table, the Sepolia cached table, the end-to-end pool rows with their measurement caveats, the Sepolia rows with the shared-engine caveat and the out-of-gas incident, and the marginal-cost micro-benchmark (`I256` 105 / `i128` 29 / `u64` 0.5 gas per pair). Reproduce: `tools/bench/bench.sh deployments/<net>.json` (devnode or Sepolia; on a cached deployment read the measured column only) and `tools/e2e/pool-e2e.sh` on a devnode.
