...
| 58 | Asset allowance per pool | `POOLS[k].asset.allowance(account, pool)`; "approved" when ≥ 1e12 (1,000,000 USDG) — on C the owner's allowance is `MAX − 1296150` after the first trade (the 1.296150 USDG pulled by `buy`; *at verification*) | 6 dp | snapshot | yes |
...
| `InsufficientFunds()` — selector `0x356680b7` (`cast sig "InsufficientFunds()"`), the **Paxos USDG token's own** error (its implementation is a facet contract, not OpenZeppelin ERC-20), bubbles from `transferFrom` on Pool C when the wallet holds less than premium + fee (reproduced 20 Sep 2026, `docs/DEMO_LOG.md` › Pool C); decoded via `REVERT_SELECTOR` in `wallet.ts` (not in the pool ABI) | Not enough USDG in your wallet for this pool — Pool C uses Paxos USDG (get 100/day at faucet.paxos.com). |
| `InsufficientAllowance()` — selector `0x13be252b` (`cast sig "InsufficientAllowance()"`), the Paxos USDG token's allowance error, bubbles from `transferFrom` on Pool C before the first approve (a `buy` on C from a wallet without allowance reverts with it — review evidence 20 Sep 2026); decoded via `REVERT_SELECTOR` | Approve USDG (Paxos) for pool C first. |
...
9. **Pool C quotes.** Its 1-unit quotes sit above A/B because a ≈ 90-USDG pool has a tiny vega cap: σ_buy on C sits at the inventory clamp (≈ 0.86 vs ≈ 0.66 on A/B at verification, +30 % vol), so the premium gap depends on moneyness — ≈ 5 % on deep-ITM rows, ≈ 30 % at the money, 2–2.5× on OTM rows — explain it as pool size/inventory, never as a math difference (the math is byte-identical to B); the Δ and Parity columns stay A vs B. The board footnote is deliberately neutral — "Pool C: same Stylus math and engine as B, settled in Paxos USDG (testnet); its inventory term (σ_mark(util)) follows its own pool size." — the wording must not freeze the state ("much smaller pool" and a single blanket percentage were true of one row on one day); say "inventory term" and let the number speak.
...
| Pool C user path (network test, block 310903851) | `asset` {A: 1999742195639, B: 1999742195639, C: 9735133} · `shares` {A: 1e12, B: 1e12, C: 90000000} · `allowance.C` = MAX − 1296150 · `positions.C` all 0 (the demo positions 5 C 2800 / 1 P 2400 exist only on A/B) |
| Pool C quotes vs A/B (dashboard, same session) | row 1 (C 2400 #0, deep ITM): Buy A 201.260127 · B 201.260574 · C 216.369340 USDG; σ_buy 0.6613 \| 0.6614 \| 0.8566; Parity ✓ — C ≈ 7.5 % above A/B on that row, inventory term at its clamp (pool size), same math. Re-measured 20 Sep 15:57 UTC (block 310919646, spot 2 610, `quoteBuy(id, 1e18)` on B vs C, board 0): C/B = 1.061× (C 2400) · 1.277× (C 2600) · 1.899× (C 2800) — the gap depends on moneyness (≈ 5 % deep ITM, ≈ 30 % ATM, 2–2.5× OTM), never a single blanket percentage |
