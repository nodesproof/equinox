...
**Status:** draft implementasi. Angka model di §6 diverifikasi numerik (Python) dan menjadi vektor uji. Pustaka Rust `bs-math`, program Stylus, dan kontrol Solidity **sudah dibangun, diuji bit-identik, dideploy ke devnode ArbOS 61/Stylus v3, dan diukur** (§13) — V2–V5, V8 dan V10 §18 terverifikasi. Program Stylus, kontrol Solidity, dan `Bench` **sudah dideploy dan di-cache di Arbitrum Sepolia** (20 Sep 2026; 20 cek bit-eksak lolos; alamat di `deployments/arbitrum-sepolia.json`). Pool (`EquinoxPool`, `EquinoxVolEngine`, `EquinoxOptionToken`, `EquinoxFactory`) sudah dibangun, di-fuzz/invariant (§12), dan dijalankan sebagai dua pool identik (kontrol vs Stylus) di devnode (§13). Alamat feed/USDG dan rubrik juri masih wajib diverifikasi Hari 1.
...
Sejak 20 Sep 2026 dua pool hidup di Arbitrum Sepolia dengan feed Chainlink ETH/USD asli dan **engine σ bersama** (K4, spec Plan 3 `docs/superpowers/specs/2026-09-20-equinox-plan3-demo-design.md`): dua engine terpisah tidak bisa tetap identik pada feed hidup karena `poke()` hanya mengamati round terakhir dan tiap trade hanya mem-poke engine pool-nya sendiri, maka engine dibuat untuk Pool B (math Stylus) dan dipakai Pool A juga lewat `EquinoxFactory.createPoolWithVol` — σ_base/σ_mark(0) identik by construction (σ_mark(util) tetap per pool), dan gas `buy`/`close` A vs B di Sepolia hanya membandingkan jalur pricing (2 panggilan `cappedCall`/`quote`), bukan seluruh lingkaran kuant, dengan program Stylus cached: setara `buy` 1 P 2.400 345.760 vs 312.273 (1,11×) dan `close` 5 C 2.800 265.191 vs 212.988 (1,25×); rasio ini tidak sebanding dengan baris devnode di atas (kolom A ikut membayar `sqrt` Stylus lewat engine bersama, kolom B tanpa premi inisialisasi), sehingga tabel devnode dua engine tetap rujukan apples-to-apples. Alamat di `deployments/arbitrum-sepolia.json`, transaksi demo (kuotasi 10 C 2.800 identik byte-per-byte pada blok 310700824 saat kedua buku masih identik, tx Arbiscan, satu insiden out-of-gas berikut post-mortem) di `docs/DEMO_LOG.md`, baris gas Sepolia beserta caveat-nya di `docs/BENCHMARK.md` ("Transaksi pool di Sepolia"). Klaim identitas live = paritas matematika pada kedua alamat `math` (K5: input identik → harga identik byte-per-byte, `onchain-check` 20/20); kuotasi pool sah berbeda lewat suku util begitu histori trade berbeda (sejak ulangan put di A: netVega Δ 1,64 → Δ kuotasi ≈ 2 × 10⁻⁵ relatif). PRD v1.4 (Plan 3b) akan merapikan §10.3–10.4 dan §13 setelah dashboard dan settlement nyata 25 Sep.
...
| V8 | ✅ Wallet testnet berdana (0,39 ETH) — deploy program + kontrol + bench + cache bid 20 Sep 2026 berhasil; faucet tidak diperlukan | `cast balance` | — |
