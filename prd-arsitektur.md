...
**Sistem tidak bergantung padanya untuk keselamatan** (T2): settle permissionless — `tools/demo/sepolia-demo.sh --claim` men-settle sendiri bila keeper belum. Status 20 Sep 2026: skrip diuji lokal (dua `poke` di atas; `DRY_RUN=1` pada 20 Sep 09:03 UTC mencetak kesehatan, `poke (simulasi)`, sequencer mock ok, dan empat baris `belum bisa settle (BoardNotExpired — expiry dalam 118/286 jam)`; setelah Pool C, `DRY_RUN=1` 20 Sep 14:00 UTC mencetak **enam** baris — board 0/1 di A, B, dan C); workflow Actions aktif sejak merge ke `main`: run cron pertama 20 Sep 2026 23:56 UTC, 148 run berturut-turut sukses sampai 22 Sep 23:11 UTC (API Actions, dicek 22 Sep 2026). Settle di Pool C yang kosong tidak revert: bounty hanya dibayar `if (bounty > 0 && freeLiquidity() >= bounty)`.

### 10.4 Dashboard + trading wallet (terbangun, Plan 3b; frontend React Plan 5)
...
**Plan 5 — frontend React (terbangun 21–22 Sep 2026).** `equinox-dashboard/` (React 19 + Vite 7 + Tailwind 4, router hash wouter, `base /equinox/`; situs statis tanpa backend, telemetri, atau skrip pihak ketiga; font di-host sendiri) menggantikan UI klasik sebagai halaman publik di URL yang sama. Logika chain tidak digandakan: `web/src/{chain,deployment.ts,abi,ui/format.ts,ui/poll.ts}` diimpor lewat alias `@chain` (tambahan kecil di sana: pembacaan `cfg()` dan `params()` lengkap, anchor waktu `DEPLOYED_AT`, unsubscribe pendengar wallet); `ChainProvider` adalah port `web/src/main.ts` — snapshot per 15 s dipaku satu blok, paritas & gas terisolasi dari kegagalan snapshot, seed lalu delta event tiap refresh ke-4, satu aksi wallet sekaligus dengan pool & akun ditangkap saat klik, cap slippage dari simulasi jalur eksekusi seperti di atas. Enam halaman: Overview, Boards, Trade, Portfolio, Activity, Contracts, plus tata letak ponsel (≤ 640 px: kartu seri, laci navigasi, target sentuh 44 px). Test: 158 test komponen/halaman (vitest + Testing Library, dirender dari fixture snapshot: live, stale, galat muat pertama, tanpa wallet, jaringan salah, pause, oracle stale, blackout, expired, settled, tanpa posisi, seed 404); bundel 762 KB mentah / 227 KB gzip JS (batas 250 KB, dicek `scripts/size.mjs` di CI). **Penerimaan:** 12 butir checklist `docs/FRONTEND_BRIEF.md` §9 dijalankan terhadap chain live pada 22 Sep 2026 22:24–22:35 UTC dan **semuanya lulus** (`equinox-dashboard/ACCEPTANCE.md`): 71/71 nilai UI sama dengan `cast call` pada blok yang sama; paritas 12/12 seri hidup; cap calldata `buy`/`close` dari wallet stub sama dengan simulasi `cast` di jendela blok klik (tidak ada yang ditandatangani); jalur RPC terputus, seed 404, dan jaringan salah; 375 px tanpa gulir horizontal halaman; Lighthouse mobile Performance 92–93 / Accessibility 100 / Best Practices 96–100; audit copy §7 21/21; Pool C (`approve` ke USDG Paxos dengan spender Pool C, `InsufficientFunds()` didekode). Karena itu `pages.yml` men-deploy aplikasi ini (`target: app`, bawaan untuk push, jadwal, dan dispatch) setelah merge ke `main`; UI klasik tetap bisa dideploy kapan saja (`gh workflow run pages.yml -f target=classic`) sebagai cadangan sampai submisi.
