...
import { POOLS, type PoolKey, type SeriesRef } from '../deployment';
...
/** Faucet MockUSDG (pool ber-`faucet === 'mint'`): 100 000 USDG (6 dp) per klik. Pool C (USDG Paxos) tidak punya mint terbuka — faucet.paxos.com, 100/hari. */
...
/** approve(pool, MAX) pada ASET pool k (A/B: MockUSDG, C: USDG Paxos asli — ERC-20 standar, ABI mock cukup untuk approve/balanceOf/allowance). */
export const approveCall = (k: PoolKey) => ({ address: POOLS[k].asset, abi: mockUsdgAbi, functionName: 'approve', args: [POOLS[k].pool, MAX_UINT] } as const);
/** mint(to, amount) pada aset pool k — hanya untuk `POOLS[k].faucet === 'mint'` (MockUSDG); pada USDG Paxos fungsi ini tidak ada. */
export const faucetCall = (k: PoolKey, to: Address, amount = FAUCET_AMOUNT) => ({ address: POOLS[k].asset, abi: mockUsdgAbi, functionName: 'mint', args: [to, amount] } as const);
/** Label aset untuk UI: `USDG (Paxos)` untuk pool ber-aset asli, `USDG (mock)` untuk pool A/B. */
export const assetLabel = (k: PoolKey) => `${POOLS[k].assetSymbol}${POOLS[k].faucet === 'paxos' ? ' (Paxos)' : ' (mock)'}`;
