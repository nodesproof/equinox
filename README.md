...
| Pools live on Sepolia (real Chainlink feed, shared vol engine, keeper, demo log) | ✅ 20 Sep 2026 — two pools, two boards, 12 series each; demo quotes byte-identical at the same block; every demo tx (incl. one out-of-gas, post-mortem included) in `docs/DEMO_LOG.md` |
...
# → (premiumAssets, feeAssets [USDG, 6 dp], sigma, delta, vegaTotal, spotWad [WAD]); reverts with SeriesExpired (then SeriesSettled)
#   once board 0 expires on 25 Sep 2026 08:00 UTC — use a board-1 id (expiry 2 Oct) after that.
...
To trade from a shell, put a funded key in a git-ignored `.env` (`SEPOLIA_PRIVATE_KEY`, optional `SEPOLIA_RPC_URL`) and run `tools/demo/sepolia-demo.sh --trade [board index]`. Dashboard and wallet trading from the browser: Plan 3b.
...
| `tools/sepolia/`, `tools/keeper/`, `tools/demo/` | Sepolia operations: `lib.sh` (`.env` + JSON, hardened `send()`), `deploy-pools.sh` (two pools, real feed, shared engine), `list-boards.sh` (seed LP + boards); `keeper.sh` (poke + settle, run by `.github/workflows/keeper.yml`); `sepolia-demo.sh` (`--trade` / `--claim`, writes `docs/DEMO_LOG.md`) |
...
| `docs/` | `BENCHMARK.md` (measured results, devnode and Sepolia), `DEMO_LOG.md` (live Sepolia demo transactions), `VERIFICATION.md` (day-1 environment checks), `superpowers/specs/` and `superpowers/plans/` (design specs and implementation plans) |
...
- **Plan 3b — web & submission:** dashboard + wallet trading (GitHub Pages), first real settlement and `--claim` on 25 Sep, video, submission package. Not built (P1): withdrawal cooldown, strike skew, `minListingDelta`, `EquinoxLens`.
