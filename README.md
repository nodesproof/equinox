...
To trade from a shell, put a funded key in a git-ignored `.env` (`SEPOLIA_PRIVATE_KEY`, optional `SEPOLIA_RPC_URL`) and run `tools/demo/sepolia-demo.sh --trade [board index]` — it compares pool quotes only while both pools carry identical inventory (`netVega`, `reserved` equal at that block); otherwise it checks math parity directly on both `math` contracts and prints the pool-quote Δ (behaviour of the next script revision, K5). Dashboard and wallet trading from the browser: Plan 3b.
