...
| Dashboard + wallet trading | ✅ <https://nodesproof.github.io/equinox/> (live since 20 Sep 2026; read-only without a wallet; trade with MetaMask on Arbitrum Sepolia) — React app in `equinox-dashboard/` (152 tests; all 12 acceptance items of the frontend brief passed against the live chain on 22 Sep 2026, [`ACCEPTANCE.md`](equinox-dashboard/ACCEPTANCE.md)) on the viem data layer in `web/` (33 unit tests + 3 network tests + a headless write smoke test) |
...
| `equinox-dashboard/` | The dashboard: React 19 + Vite 7 + Tailwind 4, static, no backend; imports the data layer from `web/src` through the `@chain` alias; six pages, 152 component/page tests (vitest + Testing Library), [`ACCEPTANCE.md`](equinox-dashboard/ACCEPTANCE.md) (brief §9 run against the live chain); published by `.github/workflows/pages.yml` — details in [`equinox-dashboard/README.md`](equinox-dashboard/README.md) |
